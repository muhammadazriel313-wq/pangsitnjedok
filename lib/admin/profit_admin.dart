...
import '/service/pdf_service.dart'; // Menjaga fungsi PDF milik teman Anda
...
class _profitAdminState extends State<profitAdmin> {
  ...
  Future<Map<String, dynamic>> fetchProfitLokal(String date) async {
    try {
      final response = await http.get(Uri.parse('${ApiService.baseUrl}/profit_admin.php?date=$date'));
            ...
            children: [
              Row(
                children: [
                  Container(
                    ...
                    child: ClipOval(
                      child: (imageUrl != null && imageUrl.isNotEmpty)
                          ? Image.network(
                              "${ApiService.baseUrl}/uploads/$imageUrl", fit: BoxFit.cover,
                              errorBuilder: (context, error, stackTrace) => Image.asset("assets/images/Dimas oi oi.jpeg", fit: BoxFit.cover),
                            )
                  ...
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start, // <-- TYPO SUDAH DIPERBAIKI
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [Text('Welcome, $adminName', style: const TextStyle(color: Color(0xFF562F00), fontSize: 16, fontWeight: FontWeight.w700, fontFamily: 'Inter'))],
                  ),
                ],
              ),
            ],
          ...
          Expanded(
            child: FutureBuilder<Map<String, dynamic>>(
              ...
              builder: (context, snapshot) {
                ...
                final List daysList = data['day_names'] ?? ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
                
                List<double> points = [];
                if (data['chart_data'] != null) {
                  points = (data['chart_data'] as List).map((e) => double.parse(e.toString())).toList();
                } else {
                ...
                return SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      // Baris Kalender & Tombol PDF Gabungan
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [
                          _buildDateCard(),
                          // Menjaga Tombol PDF teman Anda agar tetap aktif
                          IconButton(
                            icon: const Icon(Icons.picture_as_pdf, color: Color(0xFFC2410C), size: 28),
                            onPressed: () async {
                              await PdfService.generateProfitPdf(data);
                              ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(content: Text('Laporan PDF berhasil didownload!'))
                              );
                            },
                          )
                        ],
                      ...
                      Row(
                        children: [
                          Expanded(child: _buildMetricCard('Pendapatan Hari Ini', data['total_revenue'] ?? 'Rp 0')),
                          const SizedBox(width: 16),
                          Expanded(child: _buildMetricCard('Net Profit Hari Ini', data['net_profit'] ?? 'Rp 0')),
                        ],
                      ),
                      const SizedBox(height: 32),
                      
                      const Text('7 Days Trend', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF562F00))),
                      const SizedBox(height: 16),
                      _buildChart(points, daysList), 
                      const SizedBox(height: 32),
                      
                      const Text('Best Selling Hari Ini', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Color(0xFF562F00))),
                      const SizedBox(height: 16),
                      if (bestSelling.isEmpty)
                        const Padding(padding: EdgeInsets.symmetric(vertical: 20), child: Center(child: Text("Belum ada pesanan terjual hari ini", style: TextStyle(color: Colors.grey))))
                      else
                        ...bestSelling.asMap().entries.map((e) {
                          var item = e.value;
                          return _buildBestItem(e.key + 1, item['name'].toString(), int.parse(item['sold'].toString()), item['amount'].toString());
                        }),
                    ],
                  ),
                );
  ...
  Widget _buildDateCard() {
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFFC2410C), borderRadius: BorderRadius.circular(30), boxShadow: [BoxShadow(color: const Color(0xFFC2410C).withOpacity(0.3), blurRadius: 10, offset: const Offset(0, 4))]),
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Icon(Icons.calendar_month_rounded, color: Colors.white, size: 18),
            const SizedBox(width: 8),
            Text(DateFormat('dd MMMM yyyy').format(_selectedDate), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
  ...
  Widget _buildMetricCard(String title, String value) {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(24), border: Border.all(color: const Color(0xFFFFCE99)), boxShadow: const [BoxShadow(color: Color(0x0A562F00), blurRadius: 20)]),
    ...
    return Container(
      ...
      child: Column(
        children: [
          Expanded(
            child: CustomPaint(
              size: Size.infinite, // <-- PARAMETER WIDTH SUDAH DIHAPUS DAN DIGANTI INI
  ...
  Widget _buildBestItem(int rank, String name, int sold, String amount) {
    ...
    return Container(
      ...
      child: Row(children: [
        CircleAvatar(backgroundColor: top ? const Color(0xFFFF9442) : const Color(0xFFFFCE99), child: Text('$rank', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
        const SizedBox(width: 16),
  ...
  Widget _buildNavItem(String label, IconData icon, bool active, String route) {
    return GestureDetector(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8), decoration: BoxDecoration(color: active ? const Color(0xFFFFCE99) : Colors.transparent, borderRadius: BorderRadius.circular(20)), child: Icon(icon, color: const Color(0xFF562F00))),
...
class _ChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final paintFill = Paint()
      ..shader = const LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Color(0x80FF9442), Color(0x00FFFDF1)]).createShader(Rect.fromLTRB(0, 0, size.width, size.height));
    
    canvas.drawPath(fillPath, paintFill);

    final paintStroke = Paint()..color = const Color(0xFFFF9442)..style = PaintingStyle.stroke..strokeWidth = 4..strokeCap = StrokeCap.round;
    canvas.drawPath(path, paintStroke);

    // MENGGAMBAR TITIK SIMPUL (DOTS NODE)
    final paintDotFill = Paint()..color = const Color(0xFFFF9442)..style = PaintingStyle.fill;
    final paintDotBorder = Paint()..color = Colors.white..style = PaintingStyle.stroke..strokeWidth = 2;
